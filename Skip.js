...
class Skip {
  ...
  /** 清理24小时前的过期跳过记录 */
  static async cleanExpiredSkips(user_id) {
    try {
      const { executeQuery, isDbAvailable } = require('../utils/database');
      if (isDbAvailable()) {
        await executeQuery(
          'DELETE FROM skips WHERE user_id = ? AND created_at < DATE_SUB(NOW(), INTERVAL 24 HOUR)',
          [user_id]
        );
      }
    } catch (err) {
      console.error('清理过期跳过记录失败:', err.message);
    }
  }
}
