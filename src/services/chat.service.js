 ...
 *
 * 支持依赖注入（S14 单测用）：createChatService({ ...deps }) 可传入 stub 替换
...
/**
 * 创建聊天服务实例（依赖注入）
 * @param {Object} [deps] - 可注入依赖；不传则用真实模块
 */
function createChatService(deps = {}) {
  ...
  async function sendMessage({ sender_id, receiver_id, conversation_id, content, type, ...extra }) {
    ...
    // 2. 校验发送者存在
    try {
      const User = d.User || require('../models/User');
    ...
    // 11. 记录亲密度（异步）
    try {
      const intimacyService = d.intimacyService || require('./intimacy.service');
  ...
  return { sendMessage, recallMessage, buildMessageObj };
}

// 默认实例（生产使用）
const defaultInstance = createChatService();

module.exports = {
  sendMessage: defaultInstance.sendMessage,
  recallMessage: defaultInstance.recallMessage,
  buildMessageObj,
  createChatService,
};
