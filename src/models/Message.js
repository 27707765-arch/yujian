    ...
    try {
      if (isDbAvailable()) {
        ...
        if (!receiverOnline) {
          ...
          await executeQuery(
            'UPDATE conversations SET last_message = ?, last_message_time = ?, last_msg_type = ? WHERE id = ?',
            [lastText, new Date(), type, conversation_id]
          );
        }
        ...
        if (beforeId) {
          const [rows] = await executeQuery(
            `SELECT m.*, u.nickname AS sender_nickname, u.avatar AS sender_avatar,
             ...
             WHERE m.conversation_id = ? AND m.id < ? ORDER BY m.created_at DESC LIMIT ?`,
