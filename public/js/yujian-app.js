...
var DiscoverPage = {
  ...
  <div v-if="showPublish" class="card" style="padding:16px;margin-bottom:12px">
    ...
    <div v-if="pubPreview" style="position:relative;display:inline-block;margin-top:8px"><img loading="lazy" :src="pubPreview" style="max-width:200px;max-height:200px;border-radius:8px"><button @click="pubImage=null;pubPreview=''" style="position:absolute;top:-8px;right:-8px;width:24px;height:24px;border-radius:50%;background:var(--e);color:#fff;border:none;cursor:pointer;font-size:14px">✕</button></div>
  ...
  <div v-else><div v-for="p in posts" :key="p.id" class="card" style="padding:16px;margin-bottom:12px;cursor:pointer" @click="$router.push('/post/'+p.id)">
    ...
    <div v-if="p.images&&p.images.length" :style="{display:'grid',gridTemplateColumns:'repeat('+Math.min(p.images.length,3)+',1fr)',gap:'4px',marginBottom:'10px'}"><img loading="lazy" v-for="(img,i) in p.images.slice(0,9)" :src="img" style="width:100%;aspect-ratio:1;object-fit:cover;border-radius:6px"></div>
...
var PostDetailPage = {
  ...
  <div v-else>
    <div class="card" style="margin:12px 16px;padding:16px">
      ...
      <div v-if="post.images&&post.images.length" :style="{display:'grid',gridTemplateColumns:'repeat('+Math.min(post.images.length,3)+',1fr)',gap:'4px',marginBottom:'10px'}"><img loading="lazy" v-for="(img,i) in post.images" :src="img" style="width:100%;aspect-ratio:1;object-fit:cover;border-radius:6px"></div>
        ...
        <div v-for="m in msgs" :key="m.id?m.id:('k'+Math.random())">
          ...
          <div v-else class="msg-row" :style="{flexDirection:m.sender_id===userId?'row-reverse':'row'}">
            ...
            <div v-if="m.type===1" :class="['msg-b',m.sender_id===userId?'msg-my':'msg-ot']">
              <img loading="lazy" :src="m.content" class="msg-img" style="max-width:200px;border-radius:8px;display:block">
...
var AppRoot = {
  data: function(){return {toasts:toasts,uiState:uiState,appVersion:APP_VERSION,unreadCount:0}},
  computed: {
