...
var ChatDetailPage = {
  ...
  methods: {
    load: async function(isMore){this.convId=parseInt(this.$route.params.id);if(isMore){this.loadingMore=true}else{this.loading=true}try{var oldestId=isMore&&this.msgs[0]?this.msgs[0].id:null;var url=this.convId?"/chat/messages?conversation_id="+this.convId+"&limit=50":"/chat/messages?conversation_id="+this.convId+"&limit=50";if(isMore&&oldestId)url+="&before="+oldestId;var r=await api(url);var newMsgs=r.data||[];if(isMore){if(newMsgs.length===0){this.hasMore=false}else{var oldH=this.$refs.chat?this.$refs.chat.scrollHeight:0;this.msgs=newMsgs.reverse().concat(this.msgs);var s=this;this.$nextTick(function(){if(s.$refs.chat)s.$refs.chat.scrollTop=s.$refs.chat.scrollHeight-oldH})}}else{this.msgs=newMsgs.reverse();this.hasMore=newMsgs.length>=50;this.$nextTick(function(){this.scrollBottom()})}this.addTimeDividers()}catch(e){toast("加载失败","terr")}if(isMore){this.loadingMore=false}else{this.loading=false}},
