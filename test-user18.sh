...
echo "=== 用户18的同城推荐 ==="
curl -s -X GET "http://localhost:3000/api/match/recommend?scope=city&limit=10" \
  -H "Authorization: Bearer $TOKEN"

echo ""
echo ""
echo "=== 用户18的附近推荐 ==="
curl -s -X GET "http://localhost:3000/api/match/recommend?scope=nearby&distance=50&limit=10" \
  -H "Authorization: Bearer $TOKEN"
